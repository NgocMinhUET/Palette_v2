# coding=utf-8
"""
A3C networks for CL-ROI-VCM (S_INFO=9): split network / codec+ROI branches.

TensorFlow 1.x + tflearn (same backbone style as Palette a3c_agent.py).
"""
...
class ActorNetwork(object):
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        self.optimize = tf.train.RMSPropOptimizer(self.lr_rate, momentum=0.2, decay=0.9).apply_gradients(
    ...
    def create_actor_network(self):
        with tf.variable_scope("actor"):
            ...
            split_net = tf.transpose(inputs[:, 0:3, :], [0, 2, 1])
            split_net_gru = tflearn.activations.leaky_relu(
                tflearn.layers.recurrent.gru(split_net, 16, activation="linear"), alpha=leaky
            )
            split_net_flat = tflearn.flatten(split_net_gru)

            split_task = tf.transpose(inputs[:, 3:9, :], [0, 2, 1])
            split_task_gru = tflearn.activations.leaky_relu(
                tflearn.layers.recurrent.gru(split_task, 16, activation="linear"), alpha=leaky
            )
            split_task_flat = tflearn.flatten(split_task_gru)

            merge_net = tf.stack([split_net_flat, split_task_flat], axis=-1)

            dense_net = tflearn.activations.leaky_relu(
                tflearn.fully_connected(merge_net, 32, activation="linear"), alpha=leaky
            )
    ...
    def apply_gradients(self, actor_gradients):
        return self.sess.run(
            self.optimize,
            feed_dict={i: d for i, d in zip(self.actor_gradients, actor_gradients)},
        )
    ...
    def set_network_params(self, input_network_params):
        self.sess.run(
            self.set_network_params_op,
            feed_dict={i: d for i, d in zip(self.input_network_params, input_network_params)},
        )
...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        with tf.variable_scope("critic"):
            ...
            dense_net_0 = tflearn.activations.leaky_relu(
                tflearn.fully_connected(merge_net, 32, activation="linear"), alpha=leaky
            )
    ...
    def apply_gradients(self, critic_gradients):
        return self.sess.run(
            self.optimize,
            feed_dict={i: d for i, d in zip(self.critic_gradients, critic_gradients)},
        )
